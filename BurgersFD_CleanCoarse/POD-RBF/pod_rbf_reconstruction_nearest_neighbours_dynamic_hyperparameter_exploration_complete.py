...
# Function to find the best epsilon, neighbor values, and kernel per snapshot
def find_best_parameters_per_snapshot(epsilon_values, neighbor_values, kernel_types, hdm_snap, U_p, U_s, q_p_train, q_s_train, kdtree, r, print_times=False):
    ...
    for i in range(num_snapshots):
        ...
        # Test each kernel, epsilon, and neighbor combination for the current snapshot
            ...
            for epsilon in epsilon_values:
                for neighbors in neighbor_values:
                    try:
                        ...
                        print(f"Error with kernel={kernel_type}, neighbors={neighbors}, epsilon={epsilon} for snapshot {i}: {e}")

        # Store the best reconstruction and parameters for this time step
        best_reconstructed_snapshots.append(best_reconstruction)
        best_parameters_per_snapshot.append({
            'neighbors': best_neighbors,
            'epsilon': best_epsilon,
            'kernel': best_kernel
        })
        overall_error += best_error
        print(f"Snapshot {i+1}: Best kernel={best_kernel}, Best neighbors={best_neighbors}, Best epsilon={best_epsilon}, Best error={best_error:.6e}")
...
if __name__ == '__main__':
    ...
    best_reconstructed_snapshots, best_parameters_per_snapshot, overall_error = find_best_parameters_per_snapshot(
        epsilon_values, neighbor_values, kernel_types, hdm_snap, U_p, U_s, q_p_train, q_s_train, kdtree, r, print_times=False
    )
