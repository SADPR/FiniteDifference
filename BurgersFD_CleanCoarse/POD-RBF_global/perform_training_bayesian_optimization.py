...
def main():
    ...
    delta = 0.001  # Adjust this threshold as needed
    
    mask = get_duplicate_mask(q_p_train, delta)
    print(f"Removing {q_p_train.shape[0] - np.sum(mask)} near-duplicate snapshots.")

    # Apply mask and transpose back to recover the original shape
    q_p_normalized = q_p_train[mask].T  # Now q_p_normalized has shape (num_features, n_clean_snapshots)
    q_s = q_s_train[mask].T                        # Similarly, q_s now contains only the corresponding snapshots
    ...
    space = [
        Real(np.log(0.5), np.log(10), name='log_epsilon'),
