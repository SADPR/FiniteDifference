...
def calculate_epsilon_inverse_multiquadric(kdtree, q_p_train, x_new, neighbors):
    ...
    epsilon = 0.01#1 / (0.815 * d_avg)  # Formula for epsilon
    return epsilon

# Define the Inverse Multiquadratic RBF kernel function
def inverse_multiquadratic_rbf(r, epsilon):
    return 1 / np.sqrt(1 + (epsilon * r) ** 2)

# Function to dynamically interpolate at new points using nearest neighbors with Inverse Multiquadratic RBF
def interpolate_on_the_fly(kdtree, q_p_train, q_s_train, x_new, epsilon, neighbors):
    ...
    Phi_neighbors = inverse_multiquadratic_rbf(dists_neighbors, epsilon)  # Inverse Multiquadratic RBF kernel
    Phi_neighbors += np.eye(neighbors) * 1e-8  # Add small identity for stability
    W_neighbors = np.linalg.solve(Phi_neighbors, Y_neighbors)

    rbf_values = inverse_multiquadratic_rbf(dist, epsilon)  # RBF for new point
...
def reconstruct_snapshot_with_pod_rbf_neighbors(snapshot, U_p, U_s, q_p_train, q_s_train, kdtree, r, neighbors):
    q = U_p.T @ snapshot
    q_p = q[:r, :]
    ...
    for i in range(q_p.shape[1]):
        ...
        # Calculate epsilon dynamically for the specific neighbors of this point
        epsilon = calculate_epsilon_inverse_multiquadric(kdtree, q_p_train, q_p_sample, neighbors)

        # Interpolate using Inverse Multiquadratic RBF
        q_s_pred = interpolate_on_the_fly(kdtree, q_p_train, q_s_train, q_p_sample, epsilon, neighbors)
...
def find_best_epsilon_per_snapshot(hdm_snap, U_p, U_s, q_p_train, q_s_train, kdtree, r, neighbors=100):
    ...
    for i in range(num_snapshots):
        ...
        for j in range(q_p.shape[1]):
            ...
            # Calculate epsilon dynamically for this specific point's neighbors using Inverse Multiquadratic RBF
            epsilon = calculate_epsilon_inverse_multiquadric(kdtree, q_p_train, q_p_sample, neighbors)

            # Interpolate with Inverse Multiquadratic RBF
            q_s_pred = interpolate_on_the_fly(kdtree, q_p_train, q_s_train, q_p_sample, epsilon, neighbors)
            recon = U_p @ q_p[:, j] + U_s @ q_s_pred.reshape(-1)
            error = np.linalg.norm(snapshot - recon.reshape(-1, 1)) / np.linalg.norm(snapshot)
        ...
        print(f"Snapshot {i+1}: Best epsilon={best_epsilon}, Best error={best_error:.6e}")
...
if __name__ == '__main__':
    ...
    best_reconstructed_snapshots, best_parameters_per_snapshot, overall_error = find_best_epsilon_per_snapshot(
        hdm_snap, U_p, U_s, q_p_train, q_s_train, kdtree, r
    )

    # Save results
    results_dir = "Best_POD-RBF_Inverse_Multiquadric_Per_Snapshot"
    if not os.path.exists(results_dir):
        os.makedirs(results_dir)

    # Save the best reconstructed snapshots
    pod_rbf_file_path = os.path.join(results_dir, "best_reconstructed_snapshots_pod_rbf_inverse_multiquadric.npy")
    ...
    print(f"POD-RBF Reconstruction error (Inverse Multiquadric kernel): {pod_rbf_error:.6e}")
    ...
    labels = ['HDM', 'Best POD-RBF (Inverse Multiquadric)']
    colors = ['black', 'green']
    linewidths = [2, 1]
    ...
    plot_filename = os.path.join(results_dir, "comparison_plot_inverse_multiquadric.png")
