...
# Function to compute the Inverse Multiquadric RBF kernel
...
def interpolate_and_compute_jacobian_imq(kdtree, q_p_train, q_s_train, q_p_sample, epsilon, neighbors):
    ...
    # Compute the Inverse Multiquadric RBF matrix for the neighbors (Phi matrix)
    ...
    # Compute IMQ RBF values between q_p_sample and its neighbors
    rbf_values = inverse_multiquadric_rbf(dist, epsilon)
...
# Function to check the gradient of the POD-RBF model dynamically for the IMQ kernel
def gradient_check_pod_rbf_imq(U_p, snapshot_column, q_p_train, q_s_train, epsilon_values, epsilon, kdtree, neighbors):
    ...
    f_new, jacobian = interpolate_and_compute_jacobian_imq(kdtree, q_p_train, q_s_train, q_p.reshape(1, -1), epsilon, neighbors)
    ...
    for eps in epsilon_values:
        ...
        q_s_perturbed, _ = interpolate_and_compute_jacobian_imq(kdtree, q_p_train, q_s_train, q_p_perturbed.reshape(1, -1), epsilon, neighbors)
    ...
    plt.xlabel('epsilon')
    plt.ylabel('Error')
    plt.title('Gradient Check Error vs. Epsilon (Inverse Multiquadric)')
    plt.grid(True)
    plt.legend()
...
if __name__ == '__main__':
    ...
    # Perform gradient check for the POD-RBF model using the IMQ kernel
    gradient_check_pod_rbf_imq(U_p, snapshot_column, q_p_train, q_s_train, epsilon_values, epsilon, kdtree, neighbors)
