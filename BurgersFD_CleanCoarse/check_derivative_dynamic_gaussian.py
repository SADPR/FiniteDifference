...
def interpolate_and_compute_jacobian_gaussian_no_normalization(kdtree, q_p_train, q_s_train, q_p_sample, epsilon, neighbors):
    """
    Interpolate at new points and compute the Jacobian on the fly using nearest neighbors and Gaussian RBF,
    without any normalization.
    
    Parameters:
    - kdtree: KDTree built from q_p_train.
    - q_p_train: Training data for principal modes (unnormalized), shape (N_train, n_p).
    - q_s_train: Training data for secondary modes, shape (N_train, n_s).
    - q_p_sample: The input sample point (unnormalized reduced coordinates, q_p), shape (1, n_p).
    - epsilon: The width parameter for the Gaussian RBF kernel.
    - neighbors: Number of nearest neighbors to use.
    
    Returns:
    - f_new: Interpolated secondary modes at q_p_sample, shape (n_s,).
    - jacobian: Jacobian matrix of the RBF's output with respect to q_p_sample, shape (n_s, n_p).
    """
    start_time = time.time()
    
    # Ensure q_p_sample is in the correct shape
    q_p_sample = q_p_sample.reshape(1, -1)
    ...
    dist_to_sample = np.linalg.norm(X_neighbors - q_p_sample, axis=1)
    ...
    # Compute the Jacobian dynamically using a loop
    ...
    for i in range(neighbors):
        ...
        phi_r_i = rbf_values[i]  # Already computed
        if r_i > 1e-12:
            dphi_dq_p = -2 * epsilon**2 * (q_p_sample - q_p_i) * phi_r_i
        else:
            dphi_dq_p = np.zeros_like(q_p_sample)
        
        # Update the Jacobian
        jacobian += np.outer(W_neighbors[i], dphi_dq_p.flatten())
...
def gradient_check_pod_rbf_gaussian_no_normalization(U_p, snapshot_column, q_p_train, q_s_train, epsilon_values, epsilon, kdtree, neighbors):
    """
    Gradient check for the POD-RBF model using the Gaussian RBF kernel without normalization.
    
    Parameters:
    - U_p: Primary POD basis matrix, shape (n_dofs, n_p).
    - snapshot_column: Column vector from the snapshot matrix, shape (n_dofs,).
    - q_p_train: Training data for principal modes (unnormalized), shape (N_train, n_p).
    - q_s_train: Training data for secondary modes, shape (N_train, n_s).
    - epsilon_values: Array of epsilon values for the gradient check.
    - epsilon: The width parameter for the Gaussian RBF kernel.
    - kdtree: KDTree built from q_p_train.
    - neighbors: Number of nearest neighbors to use.
    
    Returns:
    - None (Plots the error vs. epsilon and prints slopes).
    """
    ...
    f_new, jacobian = interpolate_and_compute_jacobian_gaussian_no_normalization(
        kdtree, q_p_train, q_s_train, q_p.reshape(1, -1), epsilon, neighbors
    )
    ...
    for eps in epsilon_values:
        ...
        f_perturbed, _ = interpolate_and_compute_jacobian_gaussian_no_normalization(
            kdtree, q_p_train, q_s_train, q_p_perturbed.reshape(1, -1), epsilon, neighbors
        )
    ...
    plt.xlabel('epsilon')
    plt.ylabel('Error')
    plt.title('Gradient Check Error vs. Epsilon (Gaussian RBF without Normalization)')
    plt.grid(True)
    plt.legend()
...
if __name__ == '__main__':
    ...
    # Load the precomputed training data and KDTree
    with open('modes/training_data.pkl', 'rb') as f:
        data = pickle.load(f)
        kdtree = data['KDTree']
        q_p_train = data['q_p']  # Assuming these are unnormalized
        q_s_train = data['q_s']
    ...
    snapshot_column = snapshot[:, 50]
    
    # Load U_p (primary POD basis)
    U_p = np.load('modes/U_p.npy')
    
    # Define epsilon values for the gradient check
    epsilon_values = np.logspace(-12, -1, 12)
    
    # Set RBF parameters
    epsilon = 0.00001  # Adjust based on your data
    neighbors = 10
    
    # Perform gradient check for the POD-RBF model using the Gaussian RBF kernel
    gradient_check_pod_rbf_gaussian_no_normalization(
        U_p, snapshot_column, q_p_train, q_s_train, epsilon_values, epsilon, kdtree, neighbors
    )
