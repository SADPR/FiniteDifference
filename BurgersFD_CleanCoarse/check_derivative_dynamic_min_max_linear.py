...
def interpolate_and_compute_jacobian_linear_with_normalization(kdtree, q_p_train_norm, q_s_train, q_p_sample, epsilon, neighbors, scaler):
    """
    Interpolate at new points and compute the Jacobian on the fly using nearest neighbors and Linear RBF,
    ...
    # Compute the Linear RBF matrix for the neighbors (Phi matrix)
    ...
    rbf_values = linear_rbf(dist_to_sample, epsilon)
...
def gradient_check_pod_rbf_linear_with_normalization(U_p, snapshot_column, q_p_train_norm, q_s_train, epsilon_values, epsilon, kdtree, neighbors, scaler):
    """
    Gradient check for the POD-RBF model using the Linear RBF kernel with normalization.
    """
    ...
    f_new, jacobian = interpolate_and_compute_jacobian_linear_with_normalization(
    ...
    for eps in epsilon_values:
        ...
        f_perturbed, _ = interpolate_and_compute_jacobian_linear_with_normalization(
    ...
    plt.xlabel('epsilon')
    plt.ylabel('Error')
    plt.title('Gradient Check Error vs. Epsilon (Linear RBF with Normalization)')
    plt.grid(True)
    plt.legend()
...
if __name__ == '__main__':
    ...
    gradient_check_pod_rbf_linear_with_normalization(
