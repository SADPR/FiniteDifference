...
def main(mu1=5.19, mu2=0.026, compute_ecsw=True, mu_samples=None, snap_sample_factor=10):
    ...
    if compute_ecsw:
        ...
        C_cor_full = bc_w * C_full[:, (idxs == 0).ravel()]
        C_interior_full = C_full[:, (idxs == 1).ravel()]

        # Adjust the right-hand side vector
        b_cor_full = C_cor_full.sum(axis=1)
        b_interior_full = C_interior_full.sum(axis=1)

        print('Time to construct training matrix: {}'.format(time.time() - t0))
        t1 = time.time()

        # Level 0: Split the problem into two subproblems
        # Split C_interior_full and b_interior_full into two halves
        C_interior_split = np.array_split(C_interior_full, 2, axis=1)
        interior_indices_split = np.array_split(np.where(idxs.ravel() == 1)[0], 2)
        weights_interior = np.zeros(C_interior_full.shape[1])

        for idx_split, (C_interior, interior_indices) in enumerate(zip(C_interior_split, interior_indices_split)):
            print(f"Processing subproblem {idx_split+1}/2 at Level 0")
            # Level 1: Split C_interior into subdomains
            num_subdomains = 10  # Adjust as needed
            ...
            # Extract the corresponding columns from the original C_interior
            ...
            print(f"Solving NNLS at Level 2 for subproblem {idx_split+1}/2")
            w_level2, res_level2 = nnls(C_level2, b_level2)
            # Map weights back to indices in C_interior
            weights_sub_interior = np.zeros(C_interior.shape[1])
            weights_sub_interior[all_non_zero_indices] = w_level2

            # Store the weights for this subproblem
            weights_interior += weights_sub_interior

        # Level 3: Final NNLS solve combining the two subproblems
        # Extract non-zero weights and indices from Level 2 solutions
        non_zero_indices_interior = np.nonzero(weights_interior)[0]
        weights_interior_nz = weights_interior[non_zero_indices_interior]
        C_level3 = C_interior_full[:, non_zero_indices_interior]
        b_level3 = C_level3 @ weights_interior_nz

        # Solve NNLS at Level 3
        print("Solving NNLS at Level 3 (Final Level)")
        w_level3, res_level3 = nnls(C_level3, b_level3)

        # Map weights back to global indices
        weights_interior_final = np.zeros(C_interior_full.shape[1])
        weights_interior_final[non_zero_indices_interior] = w_level3

        # Handle boundary weights
        weights_boundary, res_boundary = nnls(C_cor_full, b_cor_full)

        print('Total nnls solve time: {}'.format(time.time() - t1))
        print('Level 3 nnls residual: {}'.format(res_level3))
        print('nnz(weights_interior): {}'.format(np.sum(weights_interior_final > 0)))
        print('nnz(weights_boundary): {}'.format(np.sum(weights_boundary > 0)))

        # Assemble full weights vector
        weights_full = np.zeros(C_full.shape[1])
        # Map weights_interior back to full domain indices
        interior_indices_full = np.where(idxs.ravel() == 1)[0]
        weights_full[interior_indices_full] = weights_interior_final
