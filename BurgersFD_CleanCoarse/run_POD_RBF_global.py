...
from hypernet2D import load_or_compute_snaps, plot_snaps, inviscid_burgers_pod_rbf_2D_global
...
def main(mu1=4.56, mu2=0.019, save_npy=False, save_plot=False):
    ...
    t0 = time.time()
    pod_rbf_prom_snaps, rbf_times = inviscid_burgers_pod_rbf_2D_global(
        grid_x, grid_y, w0, DT, NUM_STEPS, mu, U_p, U_s, W_global, q_p_train, q_s_train, epsilon, scaler, kernel_type=kernel_name
    )
