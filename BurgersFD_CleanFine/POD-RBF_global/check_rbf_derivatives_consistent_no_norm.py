...
def interpolate_and_compute_jacobian_normed(
    ...
    """
    q_p_sample_norm = q_p_sample_norm.reshape(-1)
...
def gradient_check_pod_rbf(
    ...
    q_p_train, W_neighbors, epsilon_values, epsilon
):
    """
    ...
    f_new_norm, jacobian_norm = interpolate_and_compute_jacobian_normed(
        kernel_type, rbf_func, q_p_train, W_neighbors,
        q_p.reshape(1, -1), epsilon
    )
    ...
    for eps in epsilon_values:
        ...
        q_p_norm_perturbed = q_p.reshape(1, -1) + eps * v
        ...
        print(f_perturbed_norm.shape)
        print(f_new_norm.shape)
        print(jacobian_norm.shape)
        print(v.shape)
...
if __name__ == '__main__':
    ...
    try:
        with open(global_weights_filename, 'rb') as f:
            ...
            q_p_train = global_data['q_p_train'] # (num_train x dim) normalized
    ...
    snapshot_file = '../param_snaps/mu1_5.19+mu2_0.026.npy'
    try:
