...
def anisotropic_dist_1toN(query_point, X_train, alpha):
    """
    query_point: shape (n_dims,) single point
    X_train: shape (N, n_dims)
    alpha: shape (n_dims,), the anisotropic scale factors

    Returns:
      distances: shape (N,) 
        distances[i] = sqrt( sum_d [ alpha[d]*(X_train[i,d] - query_point[d]) ]^2 )
    """
    scaled_query = query_point * alpha        # shape (n_dims,)
    scaled_train = X_train * alpha           # shape (N, n_dims)
    diff = scaled_train - scaled_query       # shape (N, n_dims)
    return np.sqrt(np.sum(diff**2, axis=1))  # shape (N,)


def reconstruct_snapshot_with_global_rbf(snapshot, U_p, U_s, q_p_train, W, scaler, alpha, kernel_func, print_times=False):
    ...
    for i in range(num_time_steps):
        ...
        distances = anisotropic_dist_1toN(q_p_sample.reshape(1, -1), q_p_train, alpha)#dists = np.linalg.norm(q_p_train - q_p_sample.reshape(1, -1), axis=1)
        # Apply the selected RBF kernel function
        rbf_values = kernel_func(distances, 1.0)
...
if __name__ == '__main__':
    ...
    try:
        ...
        alpha = data['alpha']     # shape (10,) if n_p=10
        kernel_name = data.get('kernel_name','matern')
        print("Loaded anisotropic RBF model successfully.")
    ...
    compare_pod = False  # Set to True to include Standard POD reconstruction
    print_times = True

    # Reconstruct the snapshot using global RBF interpolation
    pod_rbf_reconstructed = reconstruct_snapshot_with_global_rbf(
        hdm_snap, U_p, U_s, q_p_train, W, scaler, alpha, kernel_func, print_times
    )
    ...
    inds_to_plot = range(0, num_steps + 1 , 50)  # Example: every 100 time steps
