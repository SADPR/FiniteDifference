...
def main():
    ...
    epsilon_values = np.logspace(np.log10(0.1), np.log10(10.0), 10)
    lambda_values = [1e-6, 1e-4]
    ...
    for epsilon, kernel_name, lambda_val in product(epsilon_values, kernel_names, lambda_values):
        ...
        test_error = mean_squared_error(snaps_test, u_test_pred)
        test_rel_error = (np.linalg.norm(snaps_test - u_test_pred) /
                          np.linalg.norm(u_test_pred)) * 100
