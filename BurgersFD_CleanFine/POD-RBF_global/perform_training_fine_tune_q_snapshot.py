...
def main():
    ...
    for epsilon, kernel_name, lambda_val in product(epsilon_values, kernel_names, lambda_values):
        ...
        try:
            ...
            #W, residuals, rank, s = np.linalg.lstsq(Phi_train, q_s_train, rcond=None)
        ...
        #u_test_pred = (basis[:, :primary_modes] @ q_p_test_original) + (basis[:, primary_modes:total_modes] @ y_test_pred.T)

        ## Compute error metrics on test data
        #test_error = mean_squared_error(q_s_test, y_test_pred)

	# Compute the relative error row-wise (501 values)
        relative_errors = 100 * np.linalg.norm(q_s_test - y_test_pred, axis=1) / np.linalg.norm(q_s_test, axis=1)

	# Get the maximum relative error across all rows
        test_error = np.max(relative_errors)
