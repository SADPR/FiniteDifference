...
def main():
    ...
    X_train, X_val, y_train, y_val = train_test_split(q_p_train, q_s_train, test_size=0.2, random_state=42)

    # Import SVR from scikit-learn
    from sklearn.svm import SVR

    # We'll interpret "epsilon_values" as the 'C' parameter for SVR
    epsilon_values = np.logspace(np.log10(1), np.log10(5), 10)
    # Interpret kernel_names for SVR kernel
    kernel_names = ['rbf', 'linear']  # or 'poly', 'sigmoid', etc.
    ...
    print("Optimizing SVR with grid search over 'C' and 'kernel'...")
    for c_value, kernel_name in product(epsilon_values, kernel_names):
        # Create and fit the SVR model
        model = SVR(kernel=kernel_name, C=c_value, gamma='scale', epsilon=0.001)
        model.fit(X_train, y_train)  # y_train is shape (n_samples, n_secondary_modes)

        # For multi-output, we can do it dimension by dimension or do a loop. 
        # But scikit-learn's SVR is strictly for 1D y. 
        # So we can do a simple approach: average MSE across all output dims 
        # or do separate models. For simplicity, let's do an average approach below.

        y_val_pred = []
        for dim_idx in range(y_train.shape[1]):
            model_dim = SVR(kernel=kernel_name, C=c_value, gamma='scale', epsilon=0.001)
            model_dim.fit(X_train, y_train[:, dim_idx])
            y_val_pred_dim = model_dim.predict(X_val)
            y_val_pred.append(y_val_pred_dim)
        y_val_pred = np.column_stack(y_val_pred)  # shape: (n_val, n_secondary_modes)

        error = mean_squared_error(y_val, y_val_pred)
        print(f"C={c_value:.5f}, Kernel={kernel_name}, Validation MSE={error:.5e}")

        if error < lowest_error:
            lowest_error = error
            best_epsilon = c_value
            best_kernel_name = kernel_name
            best_model = None  # We'll refit below properly

    if best_epsilon is None or best_kernel_name is None:
        print("No suitable epsilon (C) and kernel combination found. Exiting.")
        return

    print(f"Best C found: {best_epsilon:.5f}")
    ...
    for dim_idx in range(q_s_train.shape[1]):
        final_svr = SVR(kernel=best_kernel_name, C=best_epsilon, gamma='scale', epsilon=0.001)
