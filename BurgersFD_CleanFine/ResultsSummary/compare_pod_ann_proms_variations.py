...
def generate_pod_ann_comparison_plot(hdm_snap, snaps_list, labels, colors, linewidths, mu1, mu2, output_image):
    """Generate and save the POD-ANN PROM vs. HPROM projection plot with a title and small margins."""
    ...
    # Add a title
    fig.suptitle(f"POD-ANN Projection for $\\mu_1 = {mu1:.2f}$, $\\mu_2 = {mu2:.3f}$", fontsize=14, y=0.98)
...
if __name__ == "__main__":
    ...
    mu1, mu2 = 4.875, 0.0225  # Example test case

    # Load HDM snapshot
    hdm_file = "../param_snaps/mu1_4.875+mu2_0.0225.npy"
    hdm_snap = np.load(hdm_file)

    # Load different PROM/HPROM reconstructions
    files = [
        "pod_ann_hprom_snaps_mu1_4.88_mu2_0.022_joshua.npy",
        "pod_ann_hprom_snaps_mu1_4.88_mu2_0.022_sebastian.npy"
    ]

    labels = [
        "POD-ANN HPROM (POD-ANN's (Joshua's) ECSW mesh)",
        "POD-ANN HPROM (POD-RBF's ECSW mesh)"
    ]
    
    colors = ["red", "green"]  # Blue for PROM, red for Joshua, green for new ECSW mesh
    linewidths = [3, 3]  # Keep all at same thickness

    # Load snapshots
    snaps_list = [np.load(file) for file in files]

    # Generate and save the plot
    output_image = f"mu1_{mu1:.2f}_mu2_{mu2:.3f}_pod_ann_hprom_comparison.png"
    generate_pod_ann_comparison_plot(hdm_snap, snaps_list, labels, colors, linewidths, mu1, mu2, output_image)
