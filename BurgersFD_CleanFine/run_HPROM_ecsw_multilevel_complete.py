...
def main(mu1=5.19, mu2=0.026, compute_ecsw=True, mu_samples=None, snap_sample_factor=10):
    ...
    if compute_ecsw:
        ...
        num_subdomains_level1 = 12  # Number of subdomains at Level 1
        C_subdomains_level1 = np.array_split(C_interior, num_subdomains_level1, axis=1)

        # Prepare start indices for Level 1 subdomains
        start_indices_level1 = np.cumsum([0] + [C_j.shape[1] for C_j in C_subdomains_level1[:-1]])
        ...
        results_level1 = Parallel(n_jobs=-1, verbose=10)(delayed(solve_nnls_subproblem)(C_i, start_idx)
                                                         for C_i, start_idx in zip(C_subdomains_level1, start_indices_level1))

        subdomain_weights_level1 = [res[0] for res in results_level1]
        subdomain_indices_level1 = [res[1] for res in results_level1]

        # Level 2: Combine the non-zero columns from Level 1
        all_non_zero_indices_level1 = np.concatenate(subdomain_indices_level1)  # Indices into C_interior
        all_non_zero_weights_level1 = np.concatenate(subdomain_weights_level1)

        # Extract the corresponding columns from C_interior to form C_level2
        C_level2 = C_interior[:, all_non_zero_indices_level1]

        # Build z_level2 (weights from Level 1)
        z_level2 = all_non_zero_weights_level1

        # Compute b_level2 by multiplying C_level2 with z_level2
        b_level2 = C_level2 @ z_level2

        # Level 2: Solve NNLS subproblems independently
        num_subdomains_level2 = 2  # Number of subdomains at Level 2
        C_subdomains_level2 = np.array_split(C_level2, num_subdomains_level2, axis=1)

        # Prepare start indices for Level 2 subdomains
        start_indices_level2 = np.cumsum([0] + [C_j.shape[1] for C_j in C_subdomains_level2[:-1]])

        # Level 2: Solve NNLS subproblems independently in parallel
        def solve_nnls_level2_subproblem(C_i_level2, start_idx):
            b_i_level2 = C_i_level2.sum(axis=1)  # Right-hand side vector for subdomain
            w_i_level2, res_i_level2 = nnls(C_i_level2, b_i_level2)
            # Store only the non-zero weights
            non_zero_indices = np.nonzero(w_i_level2)[0]
            w_i_nz = w_i_level2[non_zero_indices]
            # Map indices to global indices in C_level2
            indices_i = np.arange(start_idx, start_idx + C_i_level2.shape[1])
            indices_i_nz = indices_i[non_zero_indices]
            print(f"Level 2 subdomain starting at index {start_idx}, size of reduced mesh: {non_zero_indices.shape[0]}")
            return w_i_nz, indices_i_nz

        results_level2 = Parallel(n_jobs=-1, verbose=10)(delayed(solve_nnls_level2_subproblem)(C_i_level2, start_idx)
                                                         for C_i_level2, start_idx in zip(C_subdomains_level2, start_indices_level2))

        subdomain_weights_level2 = [res[0] for res in results_level2]
        subdomain_indices_level2 = [res[1] for res in results_level2]

        # Level 3: Combine the non-zero columns from Level 2
        all_non_zero_indices_level2 = np.concatenate(subdomain_indices_level2)  # Indices into C_level2

        # Map Level 2 indices back to indices in C_interior
        indices_level3 = all_non_zero_indices_level1[all_non_zero_indices_level2]  # Indices into C_interior
        all_non_zero_weights_level2 = np.concatenate(subdomain_weights_level2)
        weights_level3 = all_non_zero_weights_level2

        # Extract the corresponding columns from C_interior to form C_level3
        C_level3 = C_interior[:, indices_level3]

        # Build z_level3 (weights from Level 2)
        z_level3 = weights_level3

        # Compute b_level3 by multiplying C_level3 with z_level3
        b_level3 = C_level3 @ z_level3

        # Solve NNLS at Level 3
        print("Solving NNLS at Level 3")
        w_level3, res_level3 = nnls(C_level3, b_level3)
        # Map weights back to global indices (relative to C_interior)
        weights_interior = np.zeros(C_interior.shape[1])
        weights_interior[indices_level3] = w_level3

        # Handle boundary weights
        weights_boundary, res_boundary = nnls(C_cor, b_cor)
        print('Total nnls solve time: {}'.format(time.time() - t1))
        print('Level 3 nnls residual: {}'.format(res_level3))
