...
def main(mu1=4.75, mu2=0.02, compute_ecsw=False):
    ...
    if compute_ecsw:
        ...
        for i in range(0, len(C_blocks), batch_size):
            batch = C_blocks[i:i + batch_size]
            print(f"Processing batch {i // batch_size + 1}/{(len(C_blocks) + batch_size - 1) // batch_size}")
            
            # Run parallel NNLS for the current batch
            res = Parallel(n_jobs=batch_size, verbose=10)(
                delayed(nnls)(c, c.sum(axis=1), maxiter=9999999999) for c in batch
            )
            
            # Collect results
            for wi in res:
                combined_weights.append(wi[0])
        ...
        np.save('ecsw_weights_hrnm_domain_decomposition_', weights)
        plt.clf()
        plt.rcParams.update({
          "text.usetex": True,
          "mathtext.fontset": "stix",
...
#        plt.title('PROM Reduced Mesh')
        ...
        weights = np.load('ecsw_weights_hrnm_domain_decomposition_.npy')
