...
def main(mu1=4.75, mu2=0.02, compute_ecsw=False):
    ...
    if compute_ecsw:
        ...
        for i, c_block in enumerate(C_blocks):
            ...
            block_weights, _ = nnls(c_block, c_block.sum(axis=1), atol=1e-7)
        ...
        np.save('ecsw_weights_hrnm_domain_decomposition___', weights)
        plt.clf()
        plt.rcParams.update({
          "text.usetex": True,
          "mathtext.fontset": "stix",
...
#        plt.title('PROM Reduced Mesh')
        ...
        weights = np.load('ecsw_weights_hrnm_domain_decomposition___.npy')
