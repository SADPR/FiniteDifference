...
def main(mu1=4.75, mu2=0.02, compute_ecsw=False):
    ...
    if compute_ecsw:
        ...
        for imu, mu in enumerate(mu_samples):
            ...
            Ci = compute_ECSW_training_matrix_2D_rbf(snaps, prev_snaps, U_p, U_s, epsilon, neighbors,
                                        kdtree, q_p_train, q_s_train, inviscid_burgers_res2D, inviscid_burgers_exact_jac2D, grid_x, grid_y, dt, mu, kernel_type='gaussian')
            Clist.append(Ci)
    ...
    for i in range(num_time_steps):
        ...
        pod_rbf_hprom_snaps[:, i], _ = decode_rbf(q_p_snapshot, epsilon, neighbors, kdtree, q_p_train, q_s_train, U_p, U_s, kernel_type="gaussian")
