...
def main(mu1=5.19, mu2=0.026, compute_ecsw=False, save_npy=False, save_plot=False):
    ...
    if compute_ecsw:
        ...
        u,_,_,_= RandomizedSingularValueDecomposition().Calculate(C.T, 0.0)
        ...
        np.save(os.path.join(model_dir, 'ecm_weights_rbf_global.npy'), weights)
    else:
        weights = np.load(os.path.join(model_dir, 'ecm_weights_rbf_global.npy'))
